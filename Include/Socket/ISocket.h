...
namespace Socket
{
//最顶层的基类ISocket，拥有一个方法fd返回描述符，增加这个接口时由于事件中心注册到epoll里需要int型的描述符
//接下来在ISocket基础上派生出IServer与IClient两个基类。实例化出两个TCP类型的套接字类CStreamServer与CStreamClient。
class ISocket
{
	public:
		virtual ~ISocket(){};
...
class IClient: public ISocket
{
	public:
		...
		// desc: 打开套接字,对socket bind listen封装
		// param: void
		// return: 0/成功 -1/失败
		virtual int start() = 0;
